...
```
┌─────────────────────────┐       ┌────────────────────────┐
│  Модуль / BaseModule    │       │ Фоновый / Внешний скрипт│
...
Если уведомление генерируется в фоновом воркере или независимом скрипте бэкенда, где объект `ModuleContext` недоступен, следует использовать глобальную функцию `create_notification`:
...
### 4. Управление статусом прочтения
* **Отметить прочитанным**: `POST /api/notifications/{id}/read`
* **Вернуть в непрочитанные**: `POST /api/notifications/{id}/unread`
* **Отметить все прочитанными**: `POST /api/notifications/read-all`
...
### 5. Квитирование тревоги / Принятие в работу (Acknowledge)
```http
POST /api/notifications/{id}/ack
```
* Автоматически проставляет `acknowledged = 1`, имя оператора в `acknowledged_by` и штамп времени в `acknowledged_at`.
* Квитирование (`acknowledged`) и статус прочтения (`read`) являются **независимыми состояними**: оператор может принять аварию в работу (`Ack`), сохранив при этом сообщение непрочитанным для последующего контроля.
* Генерирует WebSocket-событие `notification_updated` для синхронизации интерфейса у всех подключенных операторов.
...
## 📢 7. Внешний алертинг (Alerting Engine)

Подсистема алертинга (`backend/core/alerting.py`) отправляет сообщения во внешние каналы связи независимо от внутренних уведомлений UI (`ctx.alert(...)` или `send_alert(...)`).
...
* **Стильное модальное окно**: Контрастная непрозрачная плашка Material 3 (`bg-surface-container-high`) с акцентной подсветкой `ring-1 ring-white/10`.
* **Унифицированная шапка**: Компактные икончатые кнопки управления (настройки интеграций `hub`, звук `volume`, Web Push `notifications_active`, «Прочитать все» `done_all`, «Очистить все» `delete_sweep`) с подсказками (tooltips), гарантирующими отсутствие переполнения.
* **Гибкий контроль статусов**: Возможность переключения любого сообщения между прочитанным и непрочитанным (`mark_email_unread` / `mark_email_read`), а также независимое принятие аварий в работу (`Ack` / `check_box`).

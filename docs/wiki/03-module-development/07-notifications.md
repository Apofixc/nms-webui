...
```
┌─────────────────────────┐       ┌────────────────────────┐
│  Модуль / BaseModule    │       │  Фоновый поток / Task   │
...
Если уведомление генерируется в фоновом потоке, отдельном воркере или независимом скрипте бэкенда, где объект `ModuleContext` недоступен, следует использовать глобальную функцию `create_notification`:

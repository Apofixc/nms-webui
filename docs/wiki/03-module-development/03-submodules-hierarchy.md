...
Субмодули физически располагаются внутри директории `submodules/` родительского модуля. Платформа поддерживает **однослойную структуру субмодулей** (`<module>/submodules/<submodule>`):
...
> [!NOTE]
> Загрузчик платформы (`loader.py`) при сканировании файловой системы автоматически выполняет однослойный обход директории `submodules/` и выстраивает граф зависимостей.

### Строгий однослойный режим (запрет вложенных субмодулей)
Вложенные субмодули (создание директории `submodules/` внутри самого субмодуля) **строго запрещены**:

```text
backend/modules/network_drivers/
└── submodules/
    └── cisco/                                 # Submodule (id: network_drivers.cisco)
        └── submodules/                        # ❌ Ошибка: Вложенные субмодули запрещены!
            └── ios_xr/
```

* **Обработка в Загрузчике**: Если в процессе сканирования внутри субмодуля обнаруживается папка `submodules/`, данный субмодуль не загружается, а в системе регистрируется ошибка загрузки: `register_module_error("<parent>.<submodule>", "вложенные субмодули запрещены")`.
* **Поле `parent`**: Поле `parent` в `manifest.yaml` может ссылаться только на верхнеуровневый родительский модуль и не должно содержать точек.

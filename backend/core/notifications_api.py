    ...
    try:
        ...
        try:
            ...
            try:
                ...
                loop.create_task(dispatch_notification_async(notification_dict))
            except RuntimeError:
                pass
        except Exception as exc:
            _log.warning("Could not dispatch external notification: %s", exc)
...
# ── Интеграции с внешними сервисами (Telegram, Discord, Viber, Webhooks) ──────
...
async def get_integrations():
    ...
    try:
        rows = conn.execute(
            "SELECT id, name, type, enabled, min_type, categories, config, created_at FROM notification_integrations ORDER BY created_at DESC"
        ).fetchall()
        result = []
        for r in rows:
            item = dict(r)
...
async def create_integration(payload: IntegrationPayload):
    """Создать новый канал интеграции."""
    import uuid
    conn = get_db_connection()
    try:
        ...
        with conn:
            conn.execute(
                """
                INSERT INTO notification_integrations (id, name, type, enabled, min_type, categories, config)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    integration_id,
...
async def delete_integration(integration_id: str):
    """Удалить канал интеграции."""
    conn = get_db_connection()
    try:
        with conn:
            conn.execute("DELETE FROM notification_integrations WHERE id = ?", (integration_id,))
        return {"status": "ok", "id": integration_id}
    finally:
        conn.close()
...
async def test_integration(integration_id: str):
    ...
    try:
        row = conn.execute(
            "SELECT id, name, type, config FROM notification_integrations WHERE id = ?",
            (integration_id,),
        ).fetchone()
        if not row:

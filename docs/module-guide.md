...
2. **Shared Service Registry**: Модули регистрируют свои публичные сервисы в ядре. Другие модули могут получать доступ к этим сервисам, не импортируя код модуля напрямую (Loose Coupling).
3. **Celery Bus**: Все тяжелые или межмодульные задания передаются через очередь задач Celery.
4. **Frontend Dynamic Load**: Фронтенд ничего не знает о модулях до момента загрузки. Все меню и роуты строятся динамически на основе манифестов.

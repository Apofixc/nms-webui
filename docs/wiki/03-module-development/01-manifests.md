...
Все поля валидируются Pydantic-классом `ModuleManifest` (файл `backend/core/plugin/manifest.py`).
...
Точки входа определяют, какие Python-модули и функции вызываются ядром при загрузке. Все пути указываются в формате `path.to.module:attribute` и импортируются с помощью функции `_import_from_path`.

> [!NOTE]
> **Передача контекста (`ModuleContext`)**: При вызове функций точек входа Загрузчик автоматически пытается передать объект `ctx: ModuleContext` (содержащий `module_id`, `root`, `manifest`). Благодаря механизму `_call_with_fallbacks`, функция точки входа может принимать 2 аргумента `(app, ctx)`, 1 аргумент `(ctx)` или не принимать аргументов `()`.
...
- **Автоматический жизненный цикл инстанса**:
  1. Экземпляр сохраняется в реестре инстансов (`register_instance`).

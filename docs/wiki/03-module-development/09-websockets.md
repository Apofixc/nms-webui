...
Передача JWT-токена поддерживается двумя способами:
1. **Заголовок субпротокола (`Sec-WebSocket-Protocol`)** *(Рекомендуемый)*:
   ```typescript
...
> [!IMPORTANT]
> Все WebSocket эндпоинты (`/api/events/ws` и `/api/system/logs/{log_name}/stream`) требуют обязательной аутентификации. Подключения без валидного токена автоматически отклоняются с кодом `1008 (Unauthorized)`.

### Защита от DoS и лимиты
- **Лимит подключений**: Не более 10 одновременных сокетов на один `user_id` / IP.
- **Таймаут отправки**: Бэкенд выставляет 2.0-секундный таймаут на кадр для защиты от медленных клиентов (Slowloris).

### Сохранность данных при падении бэкенда (SQLite Event Journal)
Для предотвращения потери событий при аварийном завершении бэкенда (`kill -9`, сбой питания, рестарт) применяется **SQLite Persistent Journal**:
1. **Запись на диск**: Каждое отправляемое событие фиксируется в таблице `system_events_journal` с автоинкрементным `seq_id`.
2. **Восстановление (Reconnection Replay)**:
   - При восстановлении сокета клиент посылает `{"type": "resume", "last_event_id": 1042}`.
   - Бэкенд считывает из SQLite события с `seq_id > 1042` и возвращает пакет `{"type": "replay", "events": [...]}`.
   - Если разрыв был слишком долгим и записи ротированы, сервер отправляет `{"type": "resync_required"}` для обновления полного состояния через REST API.

### Двухрежимная рассылка событий (Immediate vs Batched)

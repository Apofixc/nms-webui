...
class VLCBackend(IStreamBackend):
    """Заглушка VLC backend (disabled by default)."""
    ...
    def get_capabilities(self) -> dict[str, Any]:
        return {
            "protocols": ["udp", "http", "rtsp", "file"],
            "outputs": ["http_ts", "http_hls", "webrtc"],
            "features": ["playback"],
            "priority_matrix": {
                "udp": {"http_ts": 70, "http_hls": 70},
                "http": {"http_ts": 60, "http_hls": 60},
                "rtsp": {"http_hls": 70},
            },
        }
    ...
    def match_score(self, input_proto: str, output_format: str) -> int:
        ...
        if input_proto in caps.get("protocols", []) and output_format in caps.get("outputs", []):
            return int((caps.get("priority_matrix", {}).get(input_proto, {}) or {}).get(output_format, 40))
        return 0

    async def initialize(self, config: dict[str, Any]) -> bool:
        # Stub: mark available based on config flag, real check would probe binary
    ...
    async def process(self, task: StreamTask) -> StreamResult:
        out_dir = Path("/tmp")
        out_path = out_dir / f"{task.id}.vlc.{task.output_format}"
        out_dir.mkdir(parents=True, exist_ok=True)
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(None, out_path.write_bytes, b"")
        return StreamResult(success=True, output_path=str(out_path), backend_name="vlc")

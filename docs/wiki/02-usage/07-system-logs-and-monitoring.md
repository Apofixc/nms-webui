...
* При появлении в `backend.log` или `mcp-server.log` критических строк (`CRITICAL` или `ERROR`) подсистема `log_providers.py` формирует всплывающее Toast-уведомление и создает запись в Центре Уведомлений в шапке UI via `notifications.py`.

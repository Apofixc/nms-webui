...
### 🔑 3.1. Обязательная аутентификация и передача JWT-токена

При подключении клиенты передают JWT-токен в заголовок субпротокола `Sec-WebSocket-Protocol: bearer, <access_token>` или Query-параметре:
`wss://<host>/api/events/ws?token=<access_token>`

На бэкенде `websocket_endpoint` декодирует токен с помощью `decode_access_token(token)` и привязывает WebSocket-соединение к конкретному `user_id`. Все соединения без токена отклоняются с кодом `1008 (Unauthorized)`.

### 💾 3.2. Журналирование событий и защита от сбоев (SQLite Journal & Replay)

Для предотвращения потери данных при рестарте бэкенда каждое выходящее событие автоматически записывается в SQLite-таблицу `system_events_journal`. При восстановлении связи клиент отправляет `{ "type": "resume", "last_event_id": N }`, и бэкенд мгновенно досылает все пропущенные во время сбоя события.

### 💓 3.3. Серверный и Клиентский Heartbeat (Ping-Pong)

Для предотвращения разрывов и чистки зависших сокетов поддерживается двухсторонний Heartbeat:
- Бэкенд и клиент поддерживают таймауты ping/pong (отправка ping каждые 30 секунд, авточистка неактивных сокетов при отсутствии ответа > 60 секунд).
...
## 🔒 8. Безопасность WebSocket и защита CSWSH

Для защиты соединений реального времени от атак подделки межсайтовых сокетов (CSWSH — Cross-Site WebSocket Hijacking) и несанкционированного доступа применяются следующие правила:

1. **Динамический SECRET_KEY**: Ключ подписи JWT-токенов загружается из переменной `NMS_SECRET_KEY` или автоматически сгенерированного файла `data/secret.key` (права доступа `0600`).
2. **Защита от CSWSH**: Все WebSocket-эндпоинты проверяют заголовок `Origin` против `get_allowed_cors_origins()` (настраивается в `NMS_CORS_ORIGINS`). Запросы с чужих сайтов автоматически отклоняются с кодом `1008`.
3. **Проверка отзыва сессии (`is_session_revoked`)**:
   - Токен валидируется при подключении.
   - Каждые 30 секунд в heartbeat-цикле проверяется статус отзыва сессии в базе данных (`active_sessions`). В случае отзыва админом сокет принудительно разрывается.
4. **Безопасная передача токена на фронтенде**: Использование единого модуля `createWsClient` ([frontend/src/core/websocket.ts](file:///opt/nms-webui/frontend/src/core/websocket.ts)) гарантирует передачу JWT токена через субпротокол `Sec-WebSocket-Protocol` `['bearer', token]` вместо открытого Query-строки.

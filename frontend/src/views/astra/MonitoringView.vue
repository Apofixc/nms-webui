<template>
  <div class="p-4 sm:p-6 lg:p-8 w-full min-w-0 max-w-6xl 2xl:max-w-7xl mx-auto">
    ...
    <div v-else-if="!instances.length" class="rounded-2xl bg-surface-800/60 border border-surface-700 p-12 text-center">
      ...
      <p class="text-sm text-slate-500 mt-2">Добавьте их на вкладке
        <router-link to="/astra/instances" class="text-accent hover:underline">Управление экземплярами</router-link>.
      </p>
    </div>
    ...
    <!-- Модальное окно: перезагрузка / выключение -->
      ...
      <div class="rounded-2xl bg-surface-800 border border-surface-600 shadow-xl w-full max-w-sm p-6" @click.stop>
        ...
        <div class="flex gap-2 mb-4">
          ...
          <input
            ...
            min="0"
            max="3600"
            class="w-20 rounded bg-surface-900 border border-surface-600 px-2 py-1 text-white text-sm focus:border-accent outline-none"
          />
        </div>
        <div class="flex gap-2">
          <button
...
function virtualMemFmt(res: any) {
...
function openActionModal(inst: any, action: string) {
...
async function confirmAction() {
  ...
  try {
    if (action === 'reload') {
      await http.post(`/api/instances/${inst.id}/system/reload`, { delay })
    } else {
      await http.post(`/api/instances/${inst.id}/system/exit`, { delay })
    }
...
async function clearCache(inst: any) {
  ...
  try {
    await http.post(`/api/instances/${inst.id}/system/clear-cache`)
    await load()
  } catch {
    // ignore
  } finally {

...
5. `get_status(self) -> dict[str, Any] | ModuleStatusResponse`
   - Используется ядром платформы для отображения здоровья модуля на дашборде администрирования. Должен возвращать словарь или модель `ModuleStatusResponse` со статусом работы (`"ok"`, `"degraded"`, `"error"`), версией и деталями.

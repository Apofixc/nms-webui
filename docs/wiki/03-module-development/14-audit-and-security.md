...
Все критические и административные действия модуля (изменение параметров оборудования, сброс настроек, деактивация датчиков, удаление логов) **обязаны** регистрироваться в немодифицируемом Журнале Аудита ([backend/core/audit.py](file:///opt/nms-webui/backend/core/audit.py)).

...
> [!IMPORTANT]
> **Единственный стандарт локализации модулей**: Локализация модулей выполняется **исключительно** через JSON-файлы в директории `locales/` модуля (`locales/ru.json`, `locales/en.json`).
> Размещение блока `i18n:` внутри `manifest.yaml` запрещено во избежание дублирования переводов и раздувания файлов манифеста. Система автоматически подгружает локализации из `locales/` при старте и отдает их фронтенду по запросу `/api/modules/<module_id>/locales/<lang>`.
...
### Стандарт размещения UI-компонентов модуля
Все Vue-компоненты и страницы модуля должны размещаться в изолированной директории:
```text
frontend/src/modules/<module_id>/
├── <Module>View.vue       # Страница модуля (например, TuyaView.vue)
└── components/            # Внутренние компоненты модуля
```

Система автоматически:
1. Автоматически сканирует и регистрирует `.vue` файлы из `frontend/src/modules/<module_id>/` через `import.meta.glob`.
2. Запрашивает манифест через `/api/modules`.
3. Регистрирует пути, указанные в блоке `routes:`.
4. Отрисовывает элементы меню в сайдбаре из блока `menu:`.
5. При удалении модуля вся папка `frontend/src/modules/<module_id>/` удаляется в один шаг без сиротских файлов.

Для стандартной генерации форм настроек используется универсальный компонент `ModuleView.vue`. 

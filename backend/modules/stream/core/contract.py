...
class IStreamBackend(ABC):
    ...
    def get_temp_dirs(self, task_id: str) -> list[str]:
        """Список временных директорий/файлов, созданных бэкендом для задачи.

        Вызывается модулем Stream при очистке (release/stop).
        Субмодули НЕ ДОЛЖНЫ удалять свои временные файлы самостоятельно —
        очисткой управляет модуль.

        Returns:
            Список абсолютных или относительных путей к директориям/файлам.
        """
        return []

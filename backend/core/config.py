"""Конфигурация: список инстансов Astra из YAML + глобальные настройки."""
...
def save_instances(instances: list[InstanceConfig]) -> None:
    ...
    data = {"instances": [c.model_dump() for c in instances]}
    path.write_text(
...
def add_instance(
    host: str, port: int, api_key: str = "test", label: str | None = None
) -> InstanceConfig:
    """Добавить инстанс (если такой host:port ещё нет)."""
